...
export function ExpandableLogo({ isHome }: ExpandableLogoProps) {
  ...
  return (
    <div
      className={cn(
        "h-13 z-1 flex rounded-full items-center pl-6 pr-2 cursor-pointer transition-all duration-300 ease-in-out",
        isHome
    ...
    >
      ...
      <div
        className={cn(
          ...
          isHovered ? "opacity-100 max-w-[500px]" : "opacity-0 max-w-0"
        )}
      >
        {navItems.map((item) => (
          <div
          ...
          >
            <button
              className={cn(
                "px-4 h-full justify-center text-sm border rounded-3xl transition-all duration-200 ease-in-out whitespace-nowrap",
      ...
      <div
        className={cn(
          "h-10 flex border border-gray-300 bg-gray-100 text-[#727272] px-2.5 rounded-full self-center items-center justify-center transition-all duration-300 ease-in-out",
          isHovered ? "opacity-0 w-0 overflow-hidden" : "opacity-100 w-10"
        )}
      >
        <svg
          width="24"
